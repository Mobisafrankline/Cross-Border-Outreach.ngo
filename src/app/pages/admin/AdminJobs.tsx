        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black text-navy-900 flex items-center gap-3 font-playfair">
              <Briefcase className="w-8 h-8 text-[#F5B800]" />
              Manage Jobs
            </h1>
          </div>
          <button
            ...
            className="px-5 py-2.5 bg-[#F5B800] text-white rounded-xl font-bold flex items-center gap-2 hover:bg-[#032B45] transition-colors"
          >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {jobs.length === 0 ? (
                  ...
                  jobs.map((job) => (
                    <tr key={job.id} className="hover:bg-sky-50 transition-colors">
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button onClick={() => handleEdit(job)} className="p-2 text-[#F5B800] hover:bg-sky-50 rounded-lg transition-colors inline-block">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-900/50 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden my-8">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-sky-50">
                <h2 className="text-2xl font-bold text-navy-900 font-playfair">{editingJob ? "Edit Job" : "Post New Job"}</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full px-4 py-3 bg-sky-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-slate-700">Location</label>
                    <input required type="text" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="w-full px-4 py-3 bg-sky-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500" />
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full px-4 py-3 bg-sky-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500">
                  ...
                  <div className="space-y-2 flex flex-col justify-end">
                    <label className="flex items-center gap-3 p-3 bg-sky-50 border border-slate-200 rounded-xl cursor-pointer">
                      <input type="checkbox" checked={formData.is_active} onChange={e => setFormData({...formData, is_active: e.target.checked})} className="w-5 h-5 rounded border-slate-300 text-[#F5B800] focus:ring-blue-500" />
                ...
                <div className="space-y-2">
                  ...
                  <textarea rows={4} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full px-4 py-3 bg-sky-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500"></textarea>
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700">Requirements</label>
                  <textarea rows={3} value={formData.requirements} onChange={e => setFormData({...formData, requirements: e.target.value})} className="w-full px-4 py-3 bg-sky-50 border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500"></textarea>
                </div>
                ...
                <div className="flex gap-4 pt-4 border-t border-slate-100">
                  ...
                  <button type="submit" disabled={isSubmitting} className="flex-1 py-3 bg-[#F5B800] text-white font-bold rounded-xl hover:bg-[#032B45] transition-colors disabled:opacity-50">
