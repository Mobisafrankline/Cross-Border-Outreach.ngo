...
export default function Donate() {
  ...
  return (
    <div className="min-h-screen bg-sky-50/50">
      {/* Hero Header */}
      <div className="bg-navy-900 text-white py-16 lg:py-24 relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <motion.div 
              ...
              className="bg-white rounded-3xl shadow-xl shadow-navy-900/5 p-8 border border-slate-100"
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-navy-900 font-playfair">The Power of Your Gift</h3>
                  <p className="text-slate-500 text-sm">See how your contribution transforms lives</p>
                </div>
              </div>

              <div className="bg-sky-50 rounded-3xl p-6 border border-slate-100 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  <Globe className="w-24 h-24 text-[#F5B800]" />
                </div>
            ...
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-navy-900 px-2 flex items-center gap-2 font-playfair">
                <Gift className="w-5 h-5 text-[#F5B800]" />
                Other Ways to Give
              </h3>
              <div className="grid md:grid-cols-2 gap-4">
                <button 
                  onClick={() => setPaymentMethod("bank")}
                  className={`flex items-start gap-4 p-5 rounded-3xl border-2 transition-all text-left ${paymentMethod === "bank" ? "border-[#F5B800] bg-white shadow-lg" : "border-transparent bg-white hover:border-slate-200"}`}
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${paymentMethod === "bank" ? "bg-[#F5B800] text-white" : "bg-slate-100 text-slate-600"}`}>
                    <Building2 className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="font-bold text-navy-900">Bank & Wire Transfer</div>
                    <p className="text-sm text-slate-500">Direct deposit from any bank globally.</p>
                  </div>
                </button>
                <button 
                  onClick={() => setPaymentMethod("crypto")}
                  className={`flex items-start gap-4 p-5 rounded-3xl border-2 transition-all text-left ${paymentMethod === "crypto" ? "border-[#F5B800] bg-white shadow-lg" : "border-transparent bg-white hover:border-slate-200"}`}
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${paymentMethod === "crypto" ? "bg-[#F5B800] text-white" : "bg-slate-100 text-slate-600"}`}>
                    <Bitcoin className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="font-bold text-navy-900">Cryptocurrency</div>
              ...
              <AnimatePresence>
                {paymentMethod === "bank" && (
                  <motion.div 
                    ...
                    className="bg-[#F5B800] text-white rounded-3xl p-8 overflow-hidden"
                  >
              ...
              <AnimatePresence>
                {paymentMethod === "crypto" && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, height: "auto" }}
                    exit={{ opacity: 0, height: 0 }}
                    className="bg-navy-900 text-white rounded-3xl p-8 overflow-hidden"
                  >
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <div className="bg-navy-800 p-3 rounded-lg font-mono text-sm break-all border border-slate-700 select-all">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-24">
            <motion.div 
              ...
              className="bg-white rounded-[2rem] shadow-2xl shadow-navy-900/10 border border-slate-100 p-8 sm:p-10"
            >
              <h2 className="text-2xl font-black text-navy-900 mb-8 flex items-center gap-3 font-playfair">
                <CreditCard className="w-6 h-6 text-[#F5B800]" />
                Secure Checkout
              </h2>
              ...
              <div className="flex p-1 bg-slate-100 rounded-3xl mb-8">
                <button 
                  ...
                  className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${donationType === "one-time" ? "bg-white text-[#F5B800] shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                >
                  One-time
                </button>
                <button 
                  onClick={() => setDonationType("monthly")}
                  className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${donationType === "monthly" ? "bg-white text-[#F5B800] shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                >
              ...
              <div className="space-y-6 mb-8">
                    ...
                    <button
                      ...
                      className={`py-4 rounded-3xl font-bold text-lg border-2 transition-all ${
                        selectedAmount === amount 
                          ? "border-[#F5B800] bg-sky-50 text-[#F5B800]" 
                          : "border-slate-100 bg-sky-50 text-slate-600 hover:border-slate-200"
                      }`}
                    >
                      ${amount}
                    </button>
                ...
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                    <DollarSign className={`w-5 h-5 transition-colors ${!selectedAmount && customAmount ? "text-[#F5B800]" : "text-slate-400"}`} />
                  </div>
                  <input
                    type="number"
                    ...
                    className={`w-full bg-sky-50 pl-12 pr-6 py-5 rounded-3xl border-2 font-bold text-xl text-navy-900 transition-all focus:outline-none ${!selectedAmount && customAmount ? "border-[#F5B800] bg-white" : "border-slate-100 focus:border-blue-300"}`}
                  />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between text-sm mb-4 px-1">
                  ...
                  <span className="text-navy-900 font-black text-xl">${currentAmount.toLocaleString()}</span>
                </div>
                ...
                {currentAmount > 0 ? (
                  isStripeConfigured() ? (
                    ...
                    <div className="bg-sky-50 rounded-3xl p-6 border border-slate-100 text-center">
                      ...
                      <button 
                        onClick={() => setPaymentMethod("bank")}
                        className="w-full py-4 bg-navy-900 text-white rounded-xl font-bold hover:bg-navy-800 transition-colors flex items-center justify-center gap-2"
                      >
                  ...
                  <div className="bg-sky-50 rounded-3xl p-10 text-center border-2 border-dashed border-slate-200">
      ...
      <section className="bg-white py-24 border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-black text-navy-900 mb-4 font-playfair">Why Trust Us?</h2>
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {[
              ...
              <div key={i} className="text-center group">
                <div className="w-16 h-16 bg-sky-50 text-[#F5B800] rounded-3xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform">
                  <item.icon className="w-8 h-8" />
                </div>
                <h3 className="text-xl font-bold text-navy-900 mb-3 font-playfair">{item.title}</h3>
      ...
      {/* Bottom CTA */}
      <section className="bg-navy-900 py-20 overflow-hidden relative">
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/opportunities" className="px-8 py-4 bg-[#F5B800] text-white rounded-xl font-bold hover:bg-[#032B45] transition-all shadow-xl shadow-blue-500/20 flex items-center justify-center gap-2">
              Explore Opportunities <ArrowRight className="w-5 h-5" />
            </a>
            <a href="/fundraise" className="px-8 py-4 bg-white text-navy-900 rounded-xl font-bold hover:bg-sky-50 transition-all flex items-center justify-center gap-2">
              Start a Fundraiser
            </a>
          </div>
        </div>
